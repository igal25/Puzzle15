...
public class GameActivity15 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            sound.setCheckSound(Objects.requireNonNull(getIntent().getExtras()).getBoolean("checkSound"));
        } catch(Exception e) {
        ...
        cards = new Cards(N, N);
        try {
            if(Objects.requireNonNull(getIntent().getExtras()).getInt("keyGame") == 1) {
                continueGame();
                checkFinish();
            } else newGame();
        } catch (Exception e) {
        ...
        for(int i = 0; i < N; i++)
            for(int j = 0; j < N; j++) {
                if(cards.getValueBoard(i, j) < 10)
                    text.append("0").append(cards.getValueBoard(i, j));
                else text.append(cards.getValueBoard(i, j));
            }

        getPreferences(MODE_PRIVATE).edit().putString("savePyatnashki", text.toString()).apply();
        writeFile(Integer.toString(numbSteps), "fileScore");
        writeFile(Integer.toString(N), "fileN");
    }

    public void aboutOnClick() {
        startActivity(new Intent(GameActivity15.this, ChooseDifficultActivity.class));
    }
